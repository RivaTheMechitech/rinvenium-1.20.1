...
public class DaedalusStormshotItem extends CrossbowItem {
    private boolean charged = false;
    private boolean loaded = false;

    public DaedalusStormshotItem(Settings settings) {
        super(settings);
    }
    ...
    private static boolean loadProjectiles(LivingEntity shooter, ItemStack crossbow) {
        ...
        for (int j = 0; j < i; j++) {
            ...
            if (!loadProjectile(shooter, crossbow, itemStack, j > 0, bl)) {
                return false;
            }
        }
